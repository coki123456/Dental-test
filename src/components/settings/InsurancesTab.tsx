// src/components/settings/InsurancesTab.tsx
...
export default function InsurancesTab({ profile, handleProfileChange, handleAutoSaveProfile }: InsurancesTabProps) {
    const handleAdd = async () => {
        const el = document.getElementById('ins-input') as HTMLInputElement | null;
        if (el?.value.trim()) {
            ...
            <div className="flex flex-wrap gap-2 mb-6">
                {(profile.accepted_insurances || []).map((ins: string, i: number) => (
                    <div key={i} className="flex items-center gap-2 bg-teal-50 text-teal-700 px-4 py-2 rounded-xl text-sm border border-teal-100 font-medium shadow-sm">
                        {ins}
                        <X size={14} className="cursor-pointer hover:text-teal-900" onClick={() => handleRemove(i)} />
                    </div>
                ))}
            </div>
            <div className="flex gap-2">
                <input type="text" id="ins-input" placeholder="Agregar nueva obra social..." className="flex-1 px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-teal-500 outline-none" />
                <button onClick={handleAdd} className="px-6 py-2.5 bg-teal-600 text-white rounded-xl font-bold hover:bg-teal-700">Agregar</button>
            </div>
        </div>
    );
}
