// src/components/PacientesView.tsx
...
interface PacientesViewProps {
    ...
    onDeletePatient: (patient: NormalizedPatient | string) => Promise<void>;
...
export default function PacientesView({
    ...
    const [localPatients, setLocalPatients] = useState<NormalizedPatient[]>([]);
    const [isSearching, setIsSearching] = useState(false);

    useEffect(() => {
        if (!searchTerm) { setLocalPatients(patients); return; }
            ...
            <div className="bg-white rounded-lg shadow-sm border">
                <div className="p-4 lg:p-6 border-b flex flex-col lg:flex-row lg:justify-between lg:items-center space-y-4 lg:space-y-0">
                    ...
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center space-y-4 sm:space-y-0 sm:space-x-4">
                        <select value={statusFilter} onChange={e => setStatusFilter?.(e.target.value)} className="rounded-xl border border-transparent bg-gray-100 px-3 py-2 text-sm focus:outline-none mr-2 min-w-[120px]">
                        ...
                        <button onClick={onAddPatient} disabled={loading} className="bg-teal-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-teal-700 transition-colors w-full sm:w-auto disabled:opacity-50">
                            Agregar
                        </button>
                    </div>
                </div>
                {(loading || isSearching) ? (
                    ...
                    <PatientTable patients={filteredPacientes} onView={onViewPatient} onOpenRecord={onOpenRecord} onDelete={onDeletePatient} showActions={false} />
                )}
            </div>
        </div>
    );
