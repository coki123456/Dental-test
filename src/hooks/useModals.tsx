...
export function ModalsProvider({
    ...
    React.useEffect(() => {
        if (selectedPatient && Array.isArray(patients)) {
            ...
            if (updated) {
                const { historiaUrl: _curUrl, ...currentRest } = selectedPatient as any;
                const { historiaUrl: _newUrl2, ...newRest } = updated as any;
    ...
    React.useEffect(() => {
        if (selectedTurno && Array.isArray(turnos)) {
            const updated = turnos.find((t) => t.id === selectedTurno.id);
